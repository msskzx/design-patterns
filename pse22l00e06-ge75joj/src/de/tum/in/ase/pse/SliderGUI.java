...
// DONE Part 1.4: implement Observer<Double>
public class SliderGUI implements Observer<Double> {
	...
	/**
	 * Set this temporarily to true when setting the tempControl value during the
	 * update to avoid this adjustment to cause another update
	 */
	...
	public SliderGUI(TemperatureModel model, Point location) {
		...
		createUI();
		// DONE Part 1.6: add the object as observer to the subject
	...
	// DONE Part 1.4: Implement onUpdate:
	// invoke setValue on tempControl with the new Celsius value (converted to int)
	// to move the slider thumb
	// set updatedBySubject temporarily to true before updating to avoid creating
	// looping updates
	@Override
	public void onUpdate(Double newState) {
		this.updatedBySubject = true;
		this.tempControl.setValue(newState.intValue());
		this.updatedBySubject = false;
	}
