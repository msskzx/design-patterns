 ...
 * @see AssemblyMachine
 *      <p>
 *      It displays some general machine parameters, as well as the
 *      machine-specific ones space between transistors and chip type.
 */
public class AssemblyControlRoomView extends ControlRoomView {
	...
	public AssemblyControlRoomView(AssemblyMachine machine, AssemblyTerminal terminal) {
		...
		this.getMachine().addObserver(this);
	}
	...
	public void update(Machine currMachine) {
		 ...
		 * update this method: 2. TODO: Use the values from the machine
		 * (current-temperature, current-voltage, spaceBetweenTrans and \ chipType) and
	...
	/**
	 * Add action listeners to the setSpaceBetweenTrans & the setChipType button in
	 * order to update the machine with the input values from the respective text
	 * fields.
	 */
	protected void setButtonActions() {
		spaceBetweenSubmit
				.setOnAction(event -> terminal.setSpaceBetweenTrans(Integer.parseInt(spaceBetweenInput.getText())));
	...
	private GridPane createSpecificInputView() {
		...
		gridPane.getChildren().addAll(spaceBetweenLabel, spaceBetweenInput, spaceBetweenSubmit, chipTypeLabel,
				chipTypeInput, chipTypeSubmit);
