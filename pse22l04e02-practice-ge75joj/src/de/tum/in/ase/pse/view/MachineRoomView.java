...
/**
 * This is one actual view, representing a machine room. It displays the general
 * machine values temperature and voltage.
 */
public class MachineRoomView extends GridRoomView implements Observer {
	...
	public MachineRoomView(Machine machine, MachineTerminal terminal) {
		...
		/**
		 * We want to be notified upon change in the model, therefore we should add this
		 * as an observer to our machine 1. TODO: Add this class as an observer to the
		 * machine passed as a parameter
		 */
	 ...
	 * @see Observer
	 *      <p>
	 *      It updates displayed parameters with the current parameters of the given
	 *      machine
	 */
	@Override
	public void update(Machine currMachine) {
		/**
		 * Upon change we want to update the text in our GUI, therefore we have to
		 * update this method: 2. TODO: Use the values from the machine (current-/
		 * target-temperature, current-/ target-voltage) and \ provide them instead of
		 * the static dummy values
		 */
	...
	/**
	 * Add action listeners to the setTargetVoltage & the setTargetTemperature
	 * button in order to update the machine with the input values from the
	...
	private void setButtonActions() {
		...
		temperatureSubmit
				.setOnAction(event -> terminal.setTargetTemperature(Integer.parseInt(temperatureInput.getText())));
	}
	...
	private Label displayTitle() {
		...
		title.setBorder(new Border(new BorderStroke(Color.BLACK, BorderStrokeStyle.SOLID,
	...
	private GridPane createTargetValueView() {
		...
		grid.getChildren().addAll(targetTemperatureLabel, targetTemperatureDisplay, targetVoltageLabel,
				targetVoltageDisplay);
		return grid;
	}
	...
	private GridPane createWarningView() {
		...
		Label temperatureMinMaxMax = createLabelWithHighlight(machine.getMaxTemperature() + "",
				HIGHLIGHT_LABEL_COL_INDEX, 1);
	...
	private GridPane createBasicInputView() {
		...
		gridPane.getChildren().addAll(voltageLabel, voltageInput, voltageSubmit, temperatureLabel, temperatureInput,
				temperatureSubmit);
		return gridPane;
	}
}
