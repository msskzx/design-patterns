...
public class SuperCharger {
	...
	private static final int RECHARGE_BOUND = 100;
	...
	public void recharge(Car car) {
		...
		int luckyNumber = ThreadLocalRandom.current().nextInt(RECHARGE_BOUND);
		boolean chargingCardAccepted = luckyNumber < CHARGING_CARD_NUM; // Sometimes the different charging cards do not
																		// work everywhere
		boolean fastChargingAvailable = luckyNumber < FAST_CHARGING_NUM; // If not too many others charge atm., fast
																			// charging may be available
		if (chargingCardAccepted) {
			...
			System.out.println(
					"Car " + car.getLicensePlate() + " could not recharge, since the charging card was declined.");
		}
	}
}
